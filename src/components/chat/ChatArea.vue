  ...
  <div class="chat-area">
    <div class="chat-area__columns">
      <template v-for="botId in activeBots" :key="botId">
        <!-- 单个机器人的聊天列 -->
        <div class="chat-area__column" :style="{
          width:
          ...
          <!-- 消息列表 -->
          <div class="chat-area__messages" :ref="(el) => {
              if (el) messageContainers[botId] = el
            }
            ">
            <ChatMessage v-for="message in getBotMessages(botId)" :key="message.id" :message="message"
              :bot-name="getBotName(botId)" />
          </div>
        </div>

        <!-- 列分隔符 -->
        <div v-if="botId !== activeBots[activeBots.length - 1]" class="chat-area__column-resizer"
